# train.py
"""Training flexible Transformer model."""
...
data_creator = vmap(
    create_reg_data, in_axes=(0, None, None, None, None, None), out_axes=0
)

data_creator_ood = vmap(
    create_ood_data, in_axes=(0, None, None, None, None), out_axes=0
)

data_creator_sin_test = vmap(
    create_reg_data_sin_test, in_axes=(0, None, None, None, None), out_axes=0
)
...
def change_dataloader():
  """Selects the data generator based on the configured task."""
  global data_creator
...
def forward(tokens: jnp.ndarray, is_training: bool, gd: bool):
  """Builds the Transformer and executes a forward pass.

  NOTE:
  - Main model (gd=False) may use gated linear attention depending on Transformer/use_gla.
  - Analytic GD path (gd=True) MUST use standard MultiHeadAttention so its parameter
    names/structure match create_weights(...). Therefore we force use_gla=False for tr_gd.
  """
  if config.classic_token_const:
  ...
  tr_gd = Transformer(
      ...
      clip=config.clip,
      use_gla=False,  # CRITICAL: keep GD path on MHA to match create_weights()
      name="Transformer_gd",
  )

  return (
      tr(tokens, is_training=is_training, predict_test=False)
      if not gd
      else tr_gd(tokens, is_training=is_training, predict_test=False)
  )


def compute_loss(preds, targets):
  """Mean squared error with 1/2 factor, averaged over batch."""
...
def predict(data: jnp.ndarray, gd) -> Tuple[jnp.ndarray]:
  """Runs the model and returns raw outputs."""
...
def predict_stack(data: jnp.ndarray, gd) -> Tuple[jnp.ndarray]:
  """Runs the model and returns the intermediate prediction stack."""
...
def predict_attn(data: jnp.ndarray, gd) -> Tuple[jnp.ndarray]:
  """Runs the model and returns diagnostic attention maps."""
  _, _, attn = forward(data, False, gd)
  return attn


@hk.transform
def predict_test(data: jnp.ndarray, gd) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
  """Runs evaluation and returns final loss plus intermediate predictions."""
...
def update(state: TrainState, data, optimiser, gd=False) -> Tuple[TrainState, _Metrics]:
  """Performs one optimizer step and returns the updated state and metrics."""
...
def evaluation(train_state: TrainState, test_state: TestState, data, gd) -> TestState:
  """Evaluates the model on a batch and updates the test state."""
  rng, new_rng = jax.random.split(test_state.rng)
  loss, preds, inter_losses = predict_test.apply(train_state.params, rng, data, gd)
  new_state = TestState(
      prediction=preds,
...
def init_model(rng, train_data, test_data, optimiser) -> TrainState:
  ...
  if config.analyse:
    ...
    _ = predict_test.apply(initial_params_gd, rng, test_data, True)

  initial_test_loss, initial_preds, i_inter_losses = predict_test.apply(
      initial_params, rng, test_data, False
  )
...
def init():
  ...
  train_data = data_creator(
      jax.random.split(train_rng, num=config.bs),
      config.input_size,
      config.dataset_size,
...
def analyse_copy(data, state, rng):
  ...
  for k in range(0, len_ana):
    sum_over_od = lambda x: jnp.sum(
        predict_stack.apply(state.params, rng, x[None, ...], False)[0][0, k, :]
    )
    ...
    other += (
        (jnp.sum(grads_wrt_inputs[:k]) + jnp.sum(grads_wrt_inputs[k + 1 :]))
        / grads_wrt_inputs[:-2].shape[0]
    )
...
def analyse_gd(data, state, rng):
  ...
  loss_grad = lambda x: predict_test.apply(state.params, rng, x, False)
  grads_wrt_loss = vmap(jax.grad(loss_grad))(data[0])

  out_grad = lambda x: jnp.sum(predict.apply(state.params, rng, x, False))
...
def analyse(data, state, rng, params_constructed):
  ...
  pred = lambda z: predict.apply(state.params, rng, z[None, ...], False)[0, -1, -1]
  grads = vmap(jax.grad(pred))(data[0])[:, -1, :-1]
  predictions = vmap(pred)(data[0])

  grads_norm = jnp.linalg.norm(grads, axis=1)

  pred_c = lambda z: predict.apply(params_constructed, rng, z[None, ...], True)[0, -1, -1]
...
def compute_other_d_loss(ir, ws, rng, params, gd, bs_size=500):
  ...
  data_ood = data_creator_ood(
      jax.random.split(rng, num=bs_size),
      config.input_size,
      config.dataset_size,
      ir,
      ws,
...
def ood(state, rng, params_c, bs_size):
  ...
  Non-jitted to avoid tracer-leak / pytree-metadata hashing issues.
  """
  # python/numpy ranges are fine here because we are NOT jitting
  stretch = np.arange(0.5, 5.0 + 0.1, 0.1)
  stretch_i = np.arange(0.5, 2.0 + 0.03, 0.03)
  ...
  for i, ir in enumerate(stretch_i):
    r = jax.random.fold_in(rng, i)
    ir_t.append(compute_ood_loss(ir, config.weight_scale, r, state.params, False, bs_size))

    r = jax.random.fold_in(rng, 20_000 + i)
    ir_gd.append(compute_ood_loss(ir, config.weight_scale, r, params_c, True, bs_size))

  for i, ws in enumerate(stretch):
    r = jax.random.fold_in(rng, 10_000 + i)
    ws_t.append(compute_ood_loss(config.input_range, ws, r, state.params, False, bs_size))

    r = jax.random.fold_in(rng, 30_000 + i)
    ws_gd.append(compute_ood_loss(config.input_range, ws, r, params_c, True, bs_size))

  return jnp.array(ir_t), jnp.array(ws_t), jnp.array(ir_gd), jnp.array(ws_gd), jnp.array(stretch)
...
def ood_other_d(state, rng, params_c):
  ...
  """
  stretch = np.arange(0.5, 5.0 + 0.1, 0.1)
  stretch_i = np.arange(0.5, 5.0 + 0.05, 0.05)
  ...
  for i, ir in enumerate(stretch_i):
    r = jax.random.fold_in(rng, i)
    ir_t.append(compute_other_d_loss(ir, config.weight_scale, r, state.params, False))

    r = jax.random.fold_in(rng, 20_000 + i)
    ir_gd.append(compute_other_d_loss(ir, config.weight_scale, r, params_c, True))

  for i, ws in enumerate(stretch):
    r = jax.random.fold_in(rng, 10_000 + i)
    ws_t.append(compute_other_d_loss(config.input_range, ws, r, state.params, False))

    r = jax.random.fold_in(rng, 30_000 + i)
    ws_gd.append(compute_other_d_loss(config.input_range, ws, r, params_c, True))
...
def scan_lrs(rng, lin_diag=False, bs=10000):
  ...
  weights = lambda lr: create_weights(
      config.input_size,
      1,
      config.dataset_size,
      lr,
  ...
  eval_lr = lambda lr: compute_ood_loss(
      config.input_range, config.weight_scale, rng, weights(lr), True, bs
  )
...
def test_sin(params, rng, gd):
  ...
  eval_data = data_creator_sin_test(
      jax.random.split(test_rng, num=100),
      rng,
      config.dataset_size,
      config.input_range,
      config.weight_scale,
...
def pre_train_gd_hps(eval_rng, params_gd):
  ...
  optimiser = optax.chain(
      optax.clip_by_global_norm(config.grad_clip_value_gd),
      optax.adam(config.gd_lr, b1=0.9, b2=0.999),
  )
  opt_state = optimiser.init(params_gd)

  eval_data = data_creator(
      jax.random.split(eval_rng, num=config.bs),
      config.input_size,
      config.dataset_size,
      config.size_distract,
      config.input_range,
  ...
  for step in range(config.training_steps_gd):
    ...
    data = data_creator(
        jax.random.split(data_rng, num=config.bs_gd_train),
        config.input_size,
        config.dataset_size,
        config.size_distract,
        config.input_range,
